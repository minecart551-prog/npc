...
public class EntityAIAnimation extends Goal
{
    ...
    public int temp = AnimationType.NONE;
    ...
    public boolean canUse(){
    	...
    	if(temp != AnimationType.NONE){
    		if(!hasNavigation())
    			return npc.currentAnimation != temp;
			temp = AnimationType.NONE;
    	}
    	
    	if(hasNavigation() && !isWalkingAnimation(npc.currentAnimation)){
    		return npc.currentAnimation != AnimationType.NONE;
    	}
    ...
    public void tick(){
    	...
    	else if(temp != AnimationType.NONE){
    		if(hasNavigation())
    			temp = AnimationType.NONE;
    		else
    			type = temp;
	...
	public static int getWalkingAnimationGuiIndex(int animation) {
		if(animation == AnimationType.CROUCH)
			return 1;
