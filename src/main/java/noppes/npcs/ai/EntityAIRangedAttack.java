...
public class EntityAIRangedAttack extends Goal{
    ...
    public void tick(){
		...
		if (this.npc.ais.directLOS){
			if (this.npc.getSensing().hasLineOfSight(this.attackTarget)){
				++this.moveTries;
		    }
			else{
				this.moveTries = 0;
        ...
        if (this.rangedAttackTime-- <= 0){
            if (var1 <= (double)range && (this.npc.getSensing().hasLineOfSight(this.attackTarget) || this.npc.stats.ranged.getFireType() == 2)){
            	 ...
            	 if (this.burstCount > 1){
            		 ...
            		 switch(this.npc.stats.ranged.getFireType()){
            		     ...
            		     case 2 : indirect = !this.npc.getSensing().hasLineOfSight(this.attackTarget);
            		 }
