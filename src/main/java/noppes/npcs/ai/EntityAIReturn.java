...
public class EntityAIReturn extends Goal{
    ...
    public boolean canUse(){
    	...
    	if(npc.ais.getMovingType() == 1){
    		...
    		return !npc.isInRange(npc.getStartXPos(), -6666, npc.getStartZPos(), npc.ais.walkingRange);
    	}
    ...
    private void navigate(boolean towards) {
    	...
    	double posY = endPosY;
    	double posZ = endPosZ;
    	double range = Math.sqrt(npc.distanceToSqr(posX, posY, posZ));
    	if(range > CustomNpcs.NpcNavRange || towards){
    		...
    		if(distance > 2){
	    		...
	    		Vec3 pos = DefaultRandomPos.getPosTowards(npc, distance / 2,distance /2 > 7? 7:distance/2, start, Math.PI / 2F);
	    		if(pos != null){
	    			posX = pos.x;
	    			posY = pos.y;
	    			posZ = pos.z;
