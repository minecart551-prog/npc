...
public class QuestController implements IQuestHandler{
	...
	public void saveCategory(QuestCategory category){
		...
		categories.put(category.id, category);
		Packets.sendAll(new PacketSyncUpdate(category.id, SyncType.QUEST_CATEGORY, category.writeNBT(new CompoundTag())));
	}
	...
	public void saveQuest(QuestCategory category, Quest quest){
    	...
    	try {
			...
			if(file2.exists())
				file2.delete();
			file.renameTo(file2);
			Packets.sendAll(new PacketSyncUpdate(category.id, SyncType.QUEST, quest.save(new CompoundTag())));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
