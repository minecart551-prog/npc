...
public class GuiTextArea extends AbstractWidget implements IGui, GuiEventListener {
	...
	public void render(GuiGraphics graphics, int xMouse, int yMouse) {
        ...
        for(int i = 0; i < list.size(); i++){
        	...
        	if(startBracket != endBracket){
        		if(startBracket >= data.start && startBracket < data.end){
					...
					graphics.fill(x + 1 + s, posY, x + 1 + e, posY + container.lineHeight + 1, 0x9900cc00);
        		}
        	...
        	if(i >= scrolledLine && i < scrolledLine + container.visibleLines){
        		if(wordHightLight != null){
        			...
        			while(m.find()){
        				if(line.substring(m.start(), m.end()).equals(wordHightLight)){
                        	...
                        	int posY = y + 1 + (i - scrolledLine) * container.lineHeight;
							graphics.fill(x + 1 + s, posY, x + 1 + e, posY + container.lineHeight + 1, 0x99004c00);
        				}
        			}
        		}
            	if(startSelection != endSelection && endSelection > data.start && startSelection <= data.end){
                	if(startSelection < data.end){
                    	...
                    	int posY = y + 1 + (i - scrolledLine) * container.lineHeight;
						graphics.fill(x + 1 + s, posY, x + 1 + e, posY + container.lineHeight + 1, 0x990000ff);
                	}
            	}
            	int yPos = y + (i - scrolledLine) * container.lineHeight + 1;

                font.draw(graphics.pose(), data.getFormattedString(), x + 1, yPos, 0xFFe0e0e0);
            	...
            	if(active && isEnabled() && (cursorCounter / 6) % 2 == 0 && cursorPosition >= data.start && cursorPosition < data.end){
                    ...
                    graphics.fill(posX + 1, yPos, posX + 2, yPos + 1 + container.lineHeight, -3092272);
            	}
        	}
        }
        ...
        if(hasVerticalScrollbar()){
			...
			graphics.fill(posX, posY, posX + 5, posY + sbSize, 0xFFe0e0e0);
        }
	}
