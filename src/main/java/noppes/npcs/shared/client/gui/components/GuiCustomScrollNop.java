...
public class GuiCustomScrollNop extends Screen {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if(hasSearch) {
			textField.setX(guiLeft);
			textField.setY(guiTop);
			textField.render(graphics, mouseX, mouseY, partialTicks);
		}
		guiTop += textFieldHeight();
		mouseInList = isMouseOver(mouseX, mouseY);

		graphics.fillGradient(guiLeft, guiTop, width +guiLeft , height +guiTop, 0xc0101010, 0xd0101010);
		...
		for(int i = 0; i < list.size(); i++) {
			...
			if(k >= 4 && k + 12 < height){
				...
				if((multipleSelection && selectedList.contains(i)) || (!multipleSelection && selected == i)) {
					graphics.vLine(j-2, k-4, k + 10, 0xffffffff);
					graphics.vLine( j + width - 18 + xOffset, k - 4, k + 10, 0xffffffff);
					graphics.hLine( j - 2, j + width - 18 + xOffset, k - 3 , 0xffffffff);
					graphics.hLine( j - 2, j + width - 18 + xOffset, k + 10 , 0xffffffff);
					graphics.drawString(font, text, j , k, 0xffffff);
				}
				else if(i == hover)
					graphics.drawString(font,  text, j , k, 0x00ff00);
				else
					graphics.drawString(font, text, j , k, 0xffffff);
			}
			displayIndex++;
	...
	private void drawScrollBar(GuiGraphics graphics)
	{
		...
		int k = j;
		graphics.blit(resource, i, k, width, 9, 5, 1);
		for(k++; k < (j + scrollHeight) - 1; k++)
		{
			graphics.blit(resource, i, k, width, 10, 5, 1);
		}

		graphics.blit(resource, i, k, width, 11, 5, 1);
	}
