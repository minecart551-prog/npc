...
public class PlayerWrapper<T extends ServerPlayer> extends EntityLivingBaseWrapper<T> implements IPlayer{
	...
	public void startQuest(int id){
		...
		Packets.send((ServerPlayer)entity, new PacketAchievement(Component.translatable("quest.newquest"), Component.translatable(quest.title), 2));
		Component text = Component.translatable("quest.newquest").append(":").append(Component.translatable(quest.title));
		Packets.send((ServerPlayer)entity, new PacketChat(text));
	...
	public void sendNotification(String title, String msg, int type) {
		...
		Packets.send((ServerPlayer)entity, new PacketAchievement(Component.translatable(title), Component.translatable(msg), type));
	}
	...
	public IItemStack getInventoryHeldItem() {
		return NpcAPI.Instance().getIItemStack(entity.containerMenu.getCarried());
	}
	...
	public boolean removeItem(String id, int amount){
		Item item = (Item)BuiltInRegistries.ITEM.get(new ResourceLocation(id));
		if(item == null)
			throw new CustomNPCsException("Unknown item id: " + id);		
	...
	public boolean giveItem(IItemStack item){
		...
		if(bo){
			...
			// Inventory full — drop item on the ground at the player's position
			entity.spawnAtLocation(mcItem, 0f);
		}
		return bo;
	}
	...
	public void updatePlayerInventory() {
		...
		entity.connection.send(new ClientboundContainerSetSlotPacket(-2, 0, entity.getInventory().selected, entity.getInventory().getItem(entity.getInventory().selected)));
	...
	public void setSpawnpoint(int x, int y, int z){
        ...
        entity.setRespawnPosition(getWorld().getMCLevel().dimension(), new BlockPos(x, y, z), 0, true, false);
	}

	@Override
	public void resetSpawnpoint(){
		entity.setRespawnPosition(getWorld().getMCLevel().dimension(), null, 0, true, false);
...
//				try{
//					return CustomNpcsPermissions.hasPermission(entity, (PermissionNode<Boolean>) node);
//				}
//				catch(Throwable t){}
//			}
//		}
	...
	public void kick(String message) {
        entity.connection.disconnect(Component.translatable(message));
	}
	...
	public void showCustomGui(ICustomGui gui) {
		NoppesUtilServer.openContainerGui(this.getMCEntity(), EnumGuiType.CustomGui, (buf) -> {
			buf.writeNbt(((CustomGuiWrapper)gui).toNBT());
		});
