...
public class CustomGuiItemRendererWrapper extends CustomGuiComponentWrapper implements IItemRenderer
{
    ...
    @Override
    public int getType() {
        return GuiComponentType.ITEM_RENDERER;
    }

    @Override
    public CompoundTag toNBT(final CompoundTag nbt) {
        super.toNBT(nbt);
        nbt.put("stack", this.stack.getMCItemStack().save(new CompoundTag()));
    ...
    @Override
    public CustomGuiComponentWrapper fromNBT(final CompoundTag nbt) {
        super.fromNBT(nbt);
