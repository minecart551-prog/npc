...
public class DataAI implements INPCAi {
	...
	public int animationType = AnimationType.NONE;
	...
	public void readToNBT(CompoundTag compound) {
		...
		if(compound.contains("ActiveRange")) {
			activeRange = compound.getInt("ActiveRange");
		}
	...
	public void setStartPos(BlockPos pos) {
		startPos = pos;
		npc.restrictTo(startPos, Math.max(npc.stats.aggroRange * 2, 64));
	}
	
	public void setStartPos(IPos pos) {
		setStartPos(pos.getMCBlockPos());
	}
	
	public void setStartPos(double x, double y, double z){
		setStartPos(new BlockPos((int) x, (int) y, (int) z));
	}
	...
	public void setAvoidsWater(boolean enabled){
		npc.setPathfindingMalus(BlockPathTypes.WATER, movementType != 2 && enabled?-1.0F:0);
