...
public class DataDisplay implements INPCDisplay{
	...
	public CompoundTag save(CompoundTag nbttagcompound) {
		...
		nbttagcompound.putBoolean("OverlayGlowing", overlayGlowing);
	...
	public void readToNBT(CompoundTag nbttagcompound) {
		...
		if(nbttagcompound.contains("OverlayGlowing"))
			overlayGlowing = nbttagcompound.getBoolean("OverlayGlowing");
	...
	public void loadProfile(){
        if (this.playerProfile != null && !StringUtil.isNullOrEmpty(this.playerProfile.getName())){
			if(npc.getServer() == null){
				SkullBlockEntity.updateGameprofile(this.playerProfile, (profile) -> {
	...
	public void setBossColor(int color) {
		...
		bossColor = BossEvent.BossBarColor.values()[color];
	...
	public void setModel(String id) {
		...
		if(id == null) {
			...
			EntityType type = BuiltInRegistries.ENTITY_TYPE.get(resource);
			if(type == null)
