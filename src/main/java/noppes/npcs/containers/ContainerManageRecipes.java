...
public class ContainerManageRecipes extends AbstractContainerMenu {
    ...
    public ContainerManageRecipes(int containerId, Inventory playerInventory, int size) {
    	...
    	this.size = size * size;
    	this.width = size;
    	craftingMatrix = new SimpleContainer( this.size + 1);
	...
	public void setRecipe(RecipeCarpentry recipe, RegistryAccess access) {
    	craftingMatrix.setItem(0,recipe.getResultItem(access));
    	for(int i = 0; i < width;i++){
        	for(int j = 0; j < width;j++){
        		if(j >= recipe.getWidth())
        			craftingMatrix.setItem(i*width + j + 1, ItemStack.EMPTY);
        		else
        			craftingMatrix.setItem(i*width + j + 1, recipe.getCraftingItem(i * recipe.getWidth() + j));
        	}
    	}
    	this.recipe = recipe;
	}
