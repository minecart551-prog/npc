...
public class GuiBlockBuilder extends GuiNPCInterface implements IGuiData, ICustomScrollListener, IScrollData, BooleanConsumer {
	...
	public void save() {
		...
		Packets.sendServer(new SPacketSchematicsTileSave(pos, tile.writePartNBT(new CompoundTag())));
	}
	...
	public void setGuiData(CompoundTag compound) {
		if(compound.contains("Width")){
			...
			for(int i = 0; i < list.size(); i++){
				states.add(NbtUtils.readBlockState(player.level().holderLookup(Registries.BLOCK), list.getCompound(i)));
			}
