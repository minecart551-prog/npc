...
public class GuiQuestEdit extends GuiNPCInterface implements GuiSelectionListener, ITextfieldListener {
	...
	public void buttonEvent(GuiButtonNop guibutton){
        ...
        if(button.id == 5){
        	Packets.sendServer(new SPacketQuestOpen(EnumGuiType.QuestReward, quest.save(new CompoundTag())));
        }
        if(button.id == 6){
        ...
        if(button.id == 7){
        	if(quest.type == QuestType.ITEM)
        		Packets.sendServer(new SPacketQuestOpen(EnumGuiType.QuestItem, quest.save(new CompoundTag())));
