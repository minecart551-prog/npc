	...
	public void buttonEvent(GuiButtonNop guibutton) {
		...
		if(button.id == 12) {
            ConfirmScreen guiyesno = new ConfirmScreen((bo) -> {
				...
				NoppesUtil.openGUI(player, GuiNPCManageQuest.this);
			}, Component.translatable(selectedQuest.title), Component.translatable("gui.deleteMessage"));
            setScreen(guiyesno);
		}
        if(button.id == 13){
	...
	public void subGuiClosed(Screen subgui){
		...
		if(subgui instanceof SubGuiEditText){
			...
			if(editText.id == 3) {
				...
				Packets.sendServer(new SPacketQuestCategorySave(selectedCategory.writeNBT(new CompoundTag())));
			}
			if(editText.id == 11) {
				...
				Packets.sendServer(new SPacketQuestSave(selectedCategory.id, quest.save(new CompoundTag())));
			}
		}
