...
public class GuiNPCManageDialogs extends GuiNPCInterface2 implements ICustomScrollListener {
    ...
    public void init(){
        ...
        if(scrollDialogs == null){
        	scrollDialogs = new GuiCustomScrollNop(this,1);
	...
	public void buttonEvent(GuiButtonNop guibutton) {
		...
		if(button.id == 2) {
			...
			}, Component.translatable(selectedCategory.title), Component.translatable("gui.deleteMessage"));
            setScreen(guiyesno);
		}
        if(button.id == 3){
		...
		if(button.id == 12) {
            ConfirmScreen guiyesno = new ConfirmScreen((bo) -> {
				...
				NoppesUtil.openGUI(player, GuiNPCManageDialogs.this);
			}, Component.translatable(selectedDialog.title), Component.translatable("gui.deleteMessage"));
            setScreen(guiyesno);
		}
        if(button.id == 13){
	...
	public void subGuiClosed(Screen subgui){
		...
		if(subgui instanceof SubGuiEditText) {
			...
			if(dialogEdit.id == 3) {
				...
				Packets.sendServer(new SPacketDialogCategorySave(selectedCategory.writeNBT(new CompoundTag())));
			}
			if(dialogEdit.id == 11) {
				...
				Packets.sendServer(new SPacketDialogSave(selectedCategory.id, dialog.save(new CompoundTag())));
			}
		}
