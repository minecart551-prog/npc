...
public class GuiQuestCompletion extends GuiNPCInterface implements ITopButtonListener{
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.hLine(guiLeft + 4, guiLeft + 170, guiTop + 13,  + 0xFF000000 + CustomNpcResourceListener.DefaultTextColor);
    ...
    private void drawQuestText(GuiGraphics graphics){
    	...
    	for(int i = 0; i < block.lines.size(); i++){
    		String text = block.lines.get(i).getString();
    		 graphics.drawString(font, text, guiLeft + 4, guiTop + 16 + (i * font.lineHeight), CustomNpcResourceListener.DefaultTextColor, false);
    	}
    }
