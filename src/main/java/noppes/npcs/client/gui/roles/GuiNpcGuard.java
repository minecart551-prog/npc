...
public class GuiNpcGuard extends GuiNPCInterface2{	
    ...
    public void buttonEvent(GuiButtonNop guibutton){
        ...
        if(button.id == 0){
			for(Map.Entry<EntityType<? extends Entity>, Class> entry : EntityUtil.getAllEntitiesClasses(minecraft.level).entrySet()){
	        	...
	        	if(Animal.class.isAssignableFrom(cl) && !role.targets.contains(name)){
        ...
        if(button.id == 1){
			for(Map.Entry<EntityType<? extends Entity>, Class> entry : EntityUtil.getAllEntitiesClasses(minecraft.level).entrySet()){
				...
				Class<? extends Entity> cl = entry.getValue();
	        	String name = ent.getDescriptionId();
	        	if((Monster.class.isAssignableFrom(cl) || Enemy.class.isAssignableFrom(cl)) && !Creeper.class.isAssignableFrom(cl) && !role.targets.contains(name)){
					role.targets.add(name);
				}
        	}
			scroll1.clearSelection();
        ...
        if(button.id == 2){
			for(Map.Entry<EntityType<? extends Entity>, Class> entry : EntityUtil.getAllEntitiesClasses(minecraft.level).entrySet()){
				...
				Class<? extends Entity> cl = entry.getValue();
	        	String name = ent.getDescriptionId();
	        	if(Creeper.class.isAssignableFrom(cl) && !role.targets.contains(name)) {
					role.targets.add(name);
				}
        	}
			scroll1.clearSelection();
	...
	@Override
	public void save() {
		Packets.sendServer(new SPacketNpcJobSave(role.save(new CompoundTag())));
	}
