...
public class MpmPartReader {
    ...
    public static void reload(){
        ...
        for(ResourceLocation loc : list){
            Resource r = Minecraft.getInstance().getResourceManager().getResource(loc).orElse(null);
            if(r!=null) {
                try (InputStream stream = r.open()) {
                    ...
                    //LogWriter.error("Error in " + loc.toString(), e);
                }
            }
        }
        ANIMATIONS = mapA;
        ...
        for(Map.Entry<ResourceLocation, MpmPart> entry : PARTS.entrySet()){
            if(entry.getValue().parentId != null && !PARTS.containsKey(entry.getValue().parentId)){
                ...
                Notify(Component.literal("Error in " + entry.getKey().toString() + " - Unable to find parent " + entry.getValue().parentId));
            }
        }
    }

    private static MpmPart loadPart(ResourceLocation location){
        Resource r = Minecraft.getInstance().getResourceManager().getResource(location).orElse(null);
        if(r!=null) {
            try (InputStream stream = r.open()) {
                JsonObject root = JsonParser.parseReader(new InputStreamReader(stream, "UTF-8")).getAsJsonObject();
                ...
                if (root.has("animation_inherit")) {
                    ...
                    if (!MpmPartReader.ANIMATIONS.containsKey(inpart)) {
                        throw new NoppesException("Unknown animation inherit: " + inpart);
                    }
                    part.animationData.load(MpmPartReader.ANIMATIONS.get(inpart), part);
                }
                ...
                Notify(Component.literal("Error in " + location + " - " + e.getMessage()));
            }
        }
        return null;
    }
    ...
    public static <T extends Enum> List<T> jsonEnumList(Class<T> type, JsonElement el){
        ...
        for(int i = 0; i < arr.size(); i++){
            list.add((T) T.valueOf(type, arr.get(i).getAsString().toUpperCase()));
        }
        return list;
    }
