...
public class SkinUtil {
    ...
    public static ResourceLocation createPlayerSkin(PlayerSkinData skin) {
        ...
        //Body texture
        BufferedImage bodyImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "torsos", skin.getBodyType()));
        //Body color
        bodyImage = colorTexture(bodyImage, new Color(skin.getBodyColor()));
        //Hair texture
        BufferedImage hairImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "hairs", skin.getHairType()));
        //Hair color
        hairImage = colorTexture(hairImage, new Color(skin.getHairColor()));
        //Face texture
        BufferedImage faceImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "faces", skin.getFaceType()));
        //Eyes color
        faceImage = colorTexture(faceImage, new Color(skin.getEyesColor()));
        //Legs texture
        BufferedImage legsImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "legs", skin.getPantsType()));
        //Jacket texture
        BufferedImage jacketsImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "jackets", skin.getJacketType()));
        //Shoes texture
        BufferedImage shoesImage = readBufferedImage(rm, skin.getPartResLocByNumber(rm, "shoes", skin.getShoesType()));
        //Peculiarities texture
        for(int pec: skin.getPeculiarities()){
            listBuffers.add(readBufferedImage(rm, skin.getPartResLocByNumber(rm, "peculiarities", pec)));
        }

        //COMBINE TEXTURES
        BufferedImage skinImage = combineTextures(bodyImage, readBufferedImage(rm, skin.getPartResLocByNumber(rm, "torsos", -1)));
    ...
    private static BufferedImage readBufferedImage(ResourceManager rm, ResourceLocation resLoc) {
        ...
        try {
            imageStream = rm.getResource(resLoc).get().open();
