...
public abstract class LayerInterface extends RenderLayer{
	...
	public LayerInterface(LivingEntityRenderer render){
		super(render);
		this.render = render;
		base = (HumanoidModel) render.getModel();
	}
...
//		}
	...
	public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, Entity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch){
    	...
    	if(npc.hurtTime > 0 || npc.deathTime > 0){
			//RenderSystem.setShaderColor(1, 0, 0, 0.3f);
    	}
        if (npc.isCrouching()){
...
//	}
	...
	public abstract void render(PoseStack matrixStack, MultiBufferSource typeBuffer, int packedLightIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch);
	public abstract void rotate(PoseStack matrixStack, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch);
