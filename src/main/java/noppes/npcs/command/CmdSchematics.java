...
public class CmdSchematics {
	...
	public static final SuggestionProvider<CommandSourceStack> SCHEMAS = SuggestionProviders.register(new ResourceLocation("schemas"), (context, builder) -> {
		return SharedSuggestionProvider.suggest(names.stream(), builder);
	});

	public static final SuggestionProvider<CommandSourceStack> ROTATION = SuggestionProviders.register(new ResourceLocation("rotation"), (context, builder) -> {
		return SharedSuggestionProvider.suggest(new String[]{"0", "90", "180", "270"}, builder);
	});

	
	public static LiteralArgumentBuilder<CommandSourceStack> register() {
		LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal("schema").requires((source) -> source.hasPermission(4))
			...
			String finalS = s;
			context.getSource().sendSuccess(()->Component.translatable(finalS), false);
			return 1;
		}));
