...
public class RoleTransporter extends RoleInterface implements IRoleTransporter{
	...
	public boolean aiShouldExecute() {
		...
		List<Player> inRange = npc.level().getEntitiesOfClass(Player.class,npc.getBoundingBox().inflate(6D, 6D, 6D));
	...
	private void unlock(Player player, TransportLocation loc){
		...
		Packets.send((ServerPlayer)player, new PacketChatBubble(npc.getId(), Component.translatable("transporter.unlock", loc.name), true));
	}
